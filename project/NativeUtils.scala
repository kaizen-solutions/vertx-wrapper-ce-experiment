...
final class NativeImageGenerateMetadataFiles(
  ...
  lazy val localRepoMetadataIndex = {
    ...
    decode[List[ModuleIndexEntry]](jsonTxt)
      .valueOr(throw _)
      .map(v => (v.key, v))
      .foldLeft(Map.empty[Module, ModuleIndexEntry]) { case (acc, (module, entry)) =>
  ...
  def findAndBuildFiles(artefacts: List[Artefact]): ArtefactFiles =
    ...
    }.foldLeft(ArtefactFiles(None, None)) { (acc, res) =>
      acc ++ res
    }
...
object NativeImageGenerateMetadataFiles {
  ...
  object ModuleIndexEntry {
    implicit val json: Codec[ModuleIndexEntry] =
      Codec.forProduct4(
        ...
        "module",
        "requires"
      )(ModuleIndexEntry.apply)(m => (m.allowedPackages, m.directory, m.module, m.requires))
  }
  ...
  object ModuleVersionsIndexEntry {
    implicit val json: Codec[ModuleVersionsIndexEntry] =
      Codec.forProduct5(
        ...
        "default-for",
        "tested-versions"
      )(ModuleVersionsIndexEntry.apply)(m => (m.latest, m.metadataVersion, m.module, m.defaultFor, m.testedVersions))
  }
  ...
  object ResourcesJsonPatterns {
    implicit val json: Codec[ResourcesJsonPatterns] =
      Codec.forProduct2(
        "includes",
        "excludes"
      )(ResourcesJsonPatterns.apply)(m => (m.includes, m.excludes))
  }
  ...
  object ResourcesJson {
    implicit val json: Codec[ResourcesJson] =
      Codec.forProduct2(
        "resources",
        "bundles"
      )(ResourcesJson.apply)(m => (m.resources, m.bundles))
  }
  ...
  case class ArtefactFiles(
    ...
    def writeFilesContent(root: File)(implicit logger: Logger): List[File] =
      toFilesContent.map { case (name, content) =>
